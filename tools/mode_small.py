"""
Small-grid pathfinding visualisation mode (20x20 default).
Animated step-by-step A* / BFS / Greedy / JPS with ray-trace overlay.

Controls:
  Left-click  → set target position
  T / t       → trigger pathfinding (animated, 1 step per interval)
  R / r       → regenerate map
  F / P / E   → edit mode: target / draw walls / erase walls
  N / M       → number keys control: algorithm / map slot
  1-9         → switch algorithm or load map slot (depends on N/M)
  G           → set player position to mouse cursor
  S           → save current map
  Up / Down   → adjust walk intensity
  Esc         → quit
"""
...
import pygame
from pygame.locals import QUIT, MOUSEBUTTONDOWN, KEYDOWN, K_ESCAPE, K_t, K_UP, K_DOWN
...
logger = Logger(global_level=Level.INFO, log_to_file=True,
                log_dir=str(ROOT) + os.sep, current_pos="mode_small")
            ...
            while open_heap:
                ...
                for nb in _n8(self.world, curr):
                    ...
                    if self.algo == "weight":
                        ...
                        on_ground = not self.world.is_walkable(nb[0], nb[1] + 1)
                        if on_ground:
                            step_cost = max(0.1, step_cost - 0.7)
                        else:
                            if rdy < 0: step_cost += 2.0
                            elif rdy > 0: step_cost += 0.5
                    else:
...
def main():
    ...
    logger.info("Small-grid mode started")
    ...
    logger.info("Small-grid mode shutting down")
    logger.flush()
    pygame.quit()
    sys.exit(0)
